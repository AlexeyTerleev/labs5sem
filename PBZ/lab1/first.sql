...
left join ПРЕДМЕТ sub
	on teacher_sub.КодовыйНомерПредмета = sub.КодовыйНомерПредмета
where 99 < teacher_sub.НомерАудитории and teacher_sub.НомерАудитории < 211

/*  10   */
select array_agg(НазваниеГруппы) Группы_одной_специальности from СТУДЕНЧЕСКАЯ_ГРУППА
